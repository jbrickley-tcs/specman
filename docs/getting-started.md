...
This produces a `specman` binary on your `$PATH`. The binary is intentionally self-contained: it bundles the SpecMan data-model adapter, dependency mapper, and template engine so every run behaves the same no matter where it executes.
...
3. **Track changes with scratch pads**
   ...
   ```
   Scratch pads live under `.specman/scratchpad/` and must include front matter describing target, branch, work type (`feat`, `ref`, `revision`, or `draft`), and dependencies.
