...
- `src/crates/specman-cli/src/commands/dependencies.rs` centralizes direction parsing (`--downstream|--upstream|--all`), scope metadata, and display labels so every command group shares identical UX and error messages. The helper enforces mutual exclusivity (raising `EX_USAGE` through `CliError`) and defaults to downstream traversal when callers omit flags, matching [`spec/specman-cli/spec.md#concept-lifecycle-command-surface`](../../spec/specman-cli/spec.md#concept-lifecycle-command-surface).
- The `spec`, `impl`, and `scratch` command modules register a `dependencies` subcommand that accepts only the artifact slug (folder name). Each command validates the slug, ensures the corresponding `spec.md`, `impl.md`, or `scratch.md` exists inside the active workspace, then invokes `CliSession::dependency_mapper.dependency_tree` with the correct `ArtifactId`.
- `CommandResult::DependencyTree` plus the formatter's ASCII/JSON emitters reuse the `DependencyTree` schema already exposed by the `status` command. Text output prints deterministic ASCII trees (with cycle guards) while `--json` emits the unchanged serialized structure for automation consumers.
- Scratch pads reuse the same slug-only UX as specifications and implementations, so operators never pass raw file paths or URLs; workspace discovery resolves canonical locations, keeping the CLI in compliance with the SpecMan Data Model naming constraints.
